...
if is_accelerate_available():
    ...
    DATA_SAMPLERS = [RandomSampler, SequentialSampler]
...
class Trainer:
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if is_datasets_available() and isinstance(train_dataset, datasets.Dataset):
            pass #train_dataset = self._remove_unused_columns(train_dataset, description="training")
        else:
    ...
    def _inner_training_loop(
        ...
        if not args.ignore_data_skip:
            for epoch in range(epochs_trained):
                ...
                sampler_kinds = [RandomSampler, SequentialSampler]
                if version.parse(accelerate_version) > version.parse("0.23.0"):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        contrastive = inputs.pop("contrastive")
        ...
        triplet_loss_fct = torch.nn.TripletMarginLoss(margin=1.0, p=2, eps=1e-7)
        if all([contrastive[x][0] for x in range(len(contrastive))]) == 1:
            triplet_loss = triplet_loss_fct(outputs[1][0], outputs[1][1], outputs[1][2])
            loss += triplet_loss #todo could have a weighting factor
